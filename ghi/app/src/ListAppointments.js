...
function ListAppointments () {
    ...
    const handleCancel = (appointment) => {
        ...
        console.log(`Before delete: ${JSON.stringify(appointments)}`)
        const appointmentsUrl = `http://localhost:8080/api/appointments/${appointment.id}/`
        const requestOptions = {
            method: "DELETE",
            body: JSON.stringify(appointment),
        ...
        fetch(appointmentsUrl, requestOptions)
            .then(res => {
                setAppointments(appointments.filter(appointment => appointment !== undefined))
                return res.json()
            })
            .then(data => {
                if (data.length > ){
                ...
                // console.log(`After delete: ${JSON.stringify(appointments)}`)
            })
    }
